    ...
    for (BasicBlock *d : domby_bb) {
      if (d != bb.get()) {
        ...
        for(auto &pre : domby_bb) {
          if(pre == bb.get() || pre == d || domby[pre].find(d) == domby[pre].end()){
            continue;
          }
          all_true = false;
          break;
...
void mem2reg(Function *func){
  ...
  for(auto v : alloc_set){
    ...
    while(W.size()){
      ...
      for(auto &Y : df[bb]){
        if(F.find(Y) == F.end()){
          Y->push_front(new ir::insns::Phi(func->new_reg(v.first.type), defs[v.first], defs_reg[v.first])); // add phi
          F.insert(Y);
          bool find = false;
